...
- [ ] `2026-08-29T12:45:00Z` — `claude` — The first test in `src/server/lib/dataforseo/client.test.ts` ("skips billing in non-hosted mode") times out at the 5000ms default during a full `pnpm test`, then passes in ~1s when the file runs alone — it absorbs that file's lazy module-graph collection, which blows the budget under full-suite parallel load. Raise `testTimeout` in `vitest.config.ts` (or preload the heavy imports) so a passing suite does not depend on machine load.
